...
export default function TimelineView() {
  const { user, setUser, isPro } = useGlobalContext();
    ...
    const loadUser = async () => {
      try {
        ...
        if (userData) {
          ...
          console.log("Loaded tasks for user:", parsedUser.email, parsedUser.tasks);
        } else {
          // No user data, clear tasks
          setTasks([]);
        }
      } catch (error) {
        console.error('Error loading user data:', error);
        // On error, clear tasks
        setTasks([]);
      }
    };
  ...
  const getTaskStyle = (task, index) => {
    ...
    // Revert to standard top calculation using startMinutes directly
    const top = ((startMinutes) * HOUR_HEIGHT) / 60 + 8; // +8 for alignment
    
    // Determine styles based on completed and missed status
    ...
    if (isMissed) {
      bgColor = '#FFF1F2'; // Very light red background
      borderColor = 'rgba(239, 68, 68, 0.5)'; // Red border
      taskOpacity = 0.9; // Slightly faded
    } else if (isCompleted) {
      bgColor = '#F3F4F6'; // Light gray for completed
  ...
  // Update task completion status (via tap)
  const toggleTaskCompletion = (taskId) => {
    // Find the task first
    const taskToToggle = tasks.find(task => task.id === taskId);
    
    // If the task is marked as missed, do nothing on tap
    if (taskToToggle && taskToToggle.missed === true) {
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning); // Indicate action is blocked
      return; 
    }
    ...
    // Update in backend
    if (user && user._id) {
      axios.put('https://54ea-109-245-193-150.ngrok-free.app/updatetask', {
        taskId: taskId,
  ...
  const deleteTask = () => {
    ...
    // Update in backend
    if (user && user._id) {
      axios.delete('https://54ea-109-245-193-150.ngrok-free.app/deletetask', {
        data: {
  ...
  const handleMarkAsMissed = () => {
    ...
    const updatedTasks = tasks.map(task => 
      task.id === selectedTask.id 
        ? { ...task, completed: false, missed: true } // Set completed: false and missed: true
        : task
    );
    setTasks(updatedTasks);
    ...
    if (user && user._id) {
      axios.put('https://54ea-109-245-193-150.ngrok-free.app/updatetask', {
        ...
        missed: true,     // Add the missed flag
        userID: user._id
      })
      .then(response => {
        if (response.status === 200) {
          ...
          // Handle potential backend errors if status != 200
          console.error('Backend error updating task as missed:', response.status);
  ...
  // Clear the missed status of a task
  const handleClearMissedStatus = (taskId) => {
    Haptics.selectionAsync();
    
    if (!selectedTask || selectedTask.id !== taskId) return; // Safety check
    
    // Update local state first - only change 'missed'
    const updatedTasks = tasks.map(task => 
      task.id === taskId 
        ? { ...task, missed: false } // Set missed back to false
        : task
    );
    setTasks(updatedTasks);
    
    // Update in backend - only send 'missed: false'
    if (user && user._id) {
      axios.put('https://54ea-109-245-193-150.ngrok-free.app/updatetask', {
        taskId: taskId,
        missed: false, // Set missed to false
        // We don't change 'completed' here - it remains as it was before being missed
        userID: user._id
      })
      .then(response => {
        if (response.status === 200) {
          ...
          console.error('Backend error clearing missed status:', response.status);
          setTasks(tasks); // Revert local state
        }
      })
      .catch(error => {
        console.error('Error clearing missed status:', error);
        setTasks(tasks); // Revert local state
      });
    }
  ...
  const scrollToCurrentTime = () => {
    if (scrollViewRef.current) {
      ...
      // Revert to standard calculation using hours directly
      const totalMinutes = hours * 60 + minutes;
            
      // Calculate scroll position based on standard timeline
      const scrollPosition = ((totalMinutes / 60) * HOUR_HEIGHT) - 150; // -150 offset
  ...
  // Get current time position in timeline (standard 0-23 mapping)
  const getCurrentTimePosition = () => {
    const hours = currentTime.getHours();
    const minutes = currentTime.getMinutes();
    // Revert to standard calculation
      ...
      <View className="flex-1">
        ...
        >
          ...
          >
            ...
            >
              {/* Timeline Container - Now 25 hours tall */}
              <View style={{ height: 25 * HOUR_HEIGHT, position: 'relative' }}>
                {/* Hour Markers with Labels (00-23, then 00 again) */}
                {Array.from({ length: 25 }, (_, i) => { // Loop 25 times (0-24)
                  // Hour label loops: 0, 1, ..., 23, 0
                  const displayHour = i % 24; 
                  // Top position based on loop index
                  const topPosition = i * HOUR_HEIGHT;
                  
                  return (
                    <View 
                      key={`hour-${i}`} // Unique key 
                      style={{
                        position: 'absolute',
                        top: topPosition,
                        left: 0,
                        ...
                        height: HOUR_HEIGHT,
                        // Optional: Add styling to differentiate the bottom 00:00 visually if needed
                        // opacity: i === 24 ? 0.5 : 1, 
                      }}
                    >
                ...
                {/* Current time indicator - logic needs standard 0-23 mapping */}
                  ...
                  >
                    <View className="flex-row items-center">
                      <View className={`w-7 h-7 rounded-full items-center justify-center mr-2 ${ 
                        task.missed ? 'bg-red-100' : (task.completed ? 'bg-gray-200' : 'bg-gray-100')
                      }`}>
                        <Ionicons 
                          name={task.missed ? 'close' : (task.completed ? 'checkmark' : (task.icon || 'time-outline'))} 
                          size={14} 
                          color={task.missed ? '#EF4444' : (task.completed ? '#6B7280' : '#374151')} 
                        />
                      </View>
                      <View className="flex-1 mr-2">
                        <Text 
                          className={`font-medium text-sm ${ 
                            task.missed ? 'text-red-700' : (task.completed ? 'text-gray-500' : 'text-gray-900')
                          }`}
                          numberOfLines={1}
                        >
                          {task.text}
                        ...
                        <Text className={`text-xs ${ 
                          task.missed ? 'text-red-500' : (task.completed ? 'text-gray-400' : 'text-gray-600')
                        }`}>
        ...
        >
          <Animated.View 
            style={{
              ...
              opacity: overlayAnim.interpolate({ // Fade in backdrop
                inputRange: [0, 1],
                outputRange: [0, 1]
              }),
          ...
          >
            {/* Background blur remains the same */}
            ...
            <Animated.View 
              className="bg-white w-full rounded-t-2xl shadow-lg overflow-hidden pb-6" // Full width, top radius, bottom padding
              style={{
                transform: [
                  {
                    translateY: overlayAnim.interpolate({ // Slide up from bottom
                      inputRange: [0, 1],
                      outputRange: [SCREEN_HEIGHT * 0.5, 0] // Start halfway down, slide to 0
                    })
                  }
                ]
              }}
            >
              ...
              <View className="w-10 h-1 bg-gray-300 rounded-full self-center my-2 mb-3" />
              
              {selectedTask && (
                <View>
                  {/* --- Task Header --- */}
                  <View className="px-4 pb-3 mb-1">
                     <View className="flex-row items-center mb-1">
                       <View className={`w-8 h-8 rounded-lg items-center justify-center mr-3 ${ 
                         selectedTask.missed ? 'bg-red-100' : (selectedTask.completed ? 'bg-gray-200' : 'bg-gray-100')
                       }`}>
                         <Ionicons 
                           name={selectedTask.missed ? 'close' : (selectedTask.completed ? 'checkmark' : (selectedTask.icon || 'time-outline'))} 
                           size={18} 
                           color={selectedTask.missed ? '#EF4444' : (selectedTask.completed ? '#6B7280' : '#374151')} 
                         />
                       </View>
                  ...
                  <View className="px-2 pb-2"> 
                    ...
                    {selectedTask.missed ? (
                      <TouchableOpacity 
                        ...
                        onPress={() => handleClearMissedStatus(selectedTask.id)}
                      >
                        <View className="w-8 items-center"> 
                          <Ionicons name="refresh-circle" size={24} color="#10B981" />
                        </View>
                        <Text className="text-base text-gray-800 ml-3">Clear Missed Status</Text>
                      </TouchableOpacity>
                    ) : (
                      <View>
                        <TouchableOpacity 
                        ...
                        >
                          <View className="w-8 items-center">
                            <Ionicons name={selectedTask.completed ? "remove-circle" : "checkmark-circle"} size={24} color={selectedTask.completed ? "#EF4444" : "#10B981"} />
                          </View>
                        ...
                        >
                          ...
                          <Text className="text-base text-red-600 ml-3">Mark as Missed</Text>
                        </TouchableOpacity>
                      </View>
                    )}
                    
                    {/* Divider */} 
                    <View className="h-px bg-gray-200/75 my-2 mx-2" />
                    ...
                    >
                      ...
                      <Text className="text-base text-gray-800 ml-3">Edit Task</Text>
                    </TouchableOpacity>
                    ...
                    >
                      ...
                      <Text className="text-base text-red-600 ml-3">Delete Task</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              )}
