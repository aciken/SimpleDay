...
export default function AITaskGenerator() {
  ...
  const addSelectedTasks = async () => {
    ...
    // Create tasksToAdd with all required fields
    ...
    try {
      if (user) {
        ...
        const overlapResponse = await axios.post('https://54ea-109-245-193-150.ngrok-free.app/checkoverlap', {
          userID: user._id,
          tasks: tasksToAdd
        });

        if (overlapResponse.data.hasOverlap) {
          // Get the overlapping task IDs
          const overlappingTaskIds = overlapResponse.data.overlappingTaskIds;
          
          // Get the actual overlapping tasks for display
          const overlappingTasks = user.tasks.filter(task => 
            overlappingTaskIds.includes(task.id)
          );

          // Show alert with number of overlapping tasks and their details
          const overlappingTasksList = overlappingTasks.map(task => 
            `• ${task.text} (${task.startTime} - ${task.duration}min)`
          ).join('\n');

          Alert.alert(
            'Task Overlap Detected',
            `The following ${overlappingTasks.length} existing task(s) will be removed:\n\n${overlappingTasksList}\n\nWould you like to proceed and remove these tasks?`,
            [
              {
                text: 'Cancel',
                style: 'cancel',
              ...
              {
                text: 'Remove and Add New Tasks',
                style: 'destructive',
                onPress: async () => {
                  try {
                    const response = await axios.put('https://54ea-109-245-193-150.ngrok-free.app/addaitasks', {
                      userID: user._id,
                      tasks: tasksToAdd,
                      removeTaskIds: overlappingTaskIds
                    });

                    if (response.status === 200) {
                      // Update the user object with the new tasks and removed overlapping tasks
                      const updatedUser = {
                        ...user,
                        tasks: [...user.tasks.filter(task => !overlappingTaskIds.includes(task.id)), ...tasksToAdd]
                      };
        ...
        // If no overlaps, proceed with adding new tasks
        const response = await axios.put('https://54ea-109-245-193-150.ngrok-free.app/addaitasks', {
          userID: user._id,
          tasks: tasksToAdd
        });
    ...
    try {
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `You are a precise productivity assistant that provides data-driven insights.
            
            IMPORTANT GUIDELINES:
            - Keep responses under 150 words
            - Focus ONLY on concrete patterns from the user's task history
            - Prioritize recent tasks over older ones
            - Include specific examples with dates when making observations
            - If asked about productivity patterns, analyze completion rates and task duration
            - For time management questions, reference specific time blocks from their history
            - When uncertain, acknowledge limited data rather than making assumptions
            - If user didnt specify anything just give a general and quick few word answer, withoutu saying Based on your task history...
            
            Today's date: ${new Date().toISOString().split('T')[0]}`
          },
          {
            role: "user",
            content: `Here is my task history: ${JSON.stringify(taskHistory)}\n\nQuestion: ${prompt}`
          }
        ],
        temperature: 0.3,
        max_tokens: 500
